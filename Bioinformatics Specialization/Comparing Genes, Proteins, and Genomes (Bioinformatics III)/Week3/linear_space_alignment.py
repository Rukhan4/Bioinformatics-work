...
blosum_62 = {'A': {'A': 4, 'C': 0, 'E': -1, 'D': -2, 'G': 0, 'F': -2, 'I': -1, 'H': -2, 'K': -1, 'M': -1, 'L': -1, 'N': -2, 'Q': -1, 'P': -1, 'S': 1, 'R': -1, 'T': 0, 'W': -3, 'V': 0, 'Y': -2},
...
with open(r'C:\Users\18687\Desktop\Bio Informatics\Bioinformatics specialization\testsets\linearspacealignmenttest.txt', 'r') as file:
    v = file.readline().strip()
    w = file.readline().strip()


def get_backtrack_with_score(v, w):
    n = len(v)
    m = len(w)
    l = [[''] * (m + 1) for i in range(n + 1)]
    backtrack = np.matrix(l)
    s = np.zeros((n + 1, m + 1))
    ...
    for i in range(1, n + 1):
        for j in range(1, m + 1):
            s[i, j] = max(s[i - 1, j - 1] + blosum_62[v[i - 1]]
                          [w[j - 1]], s[i - 1, j] - 5, s[i, j - 1] - 5)
            if s[i, j] == s[i - 1, j] - 5:
                backtrack[i, j] = '↓'
            elif s[i, j] == s[i, j - 1] - 5:
                backtrack[i, j] = '→'
            elif s[i, j] == s[i - 1, j - 1] + blosum_62[v[i - 1]][w[j - 1]]:
                backtrack[i, j] = '↘'
    return backtrack, int(s[n, m])


def insert_minus(word, index):
...
v_with_minuses = v
w_with_minuses = w
backtrack, score = get_backtrack_with_score(v, w)
while len_v != 0 and len_w != 0:
    if backtrack[len_v, len_w] == '↓':
        len_v -= 1
        w_with_minuses = insert_minus(w_with_minuses, len_w)
    elif backtrack[len_v, len_w] == '→':
        len_w -= 1
        v_with_minuses = insert_minus(v_with_minuses, len_v)
    else:
        len_v -= 1
        len_w -= 1
for i in range(len_v):
    w_with_minuses = insert_minus(w_with_minuses, 0)
for j in range(len_w):
    v_with_minuses = insert_minus(v_with_minuses, 0)


print(score)
print(v_with_minuses)
print(w_with_minuses)
